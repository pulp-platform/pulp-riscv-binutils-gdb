/* RISC-V disassembler
   ...
   PULP family support contributed by Eric Flamand (eflamand@iis.ee.ethz.ch) at
   ETH-Zurich and Greenwaves Technologies
...
static void riscv_set_arch(const char *arg) {
  ...
  default:
    fprintf(stderr,
            "`I' must be the first ISA subset name specified (got %c). "
            "Ignoring -march",
            *p);
    riscv_subsets = NULL;
    return;
  ...
  while (*p) {
    if (*p == 'X') {
      ...
      *q = '\0';
      ...
      if (extension) {
        fprintf(stderr,
                "only one eXtension is supported (found %s and %s). Ignoring "
                "-march",
                extension, subset);
        riscv_subsets = NULL;
        return;
...
static void parse_riscv_dis_option(const char *option) {
  ...
  if (strcmp(option, "no-aliases") == 0)
    ...
    for (i = 0; uppercase[i]; i++)
      uppercase[i] = toupper(uppercase[i]);
    if (strstr(uppercase, "PULPINO"))
      riscv_set_arch("RV32IMCXpulpv1");
    else if (strstr(uppercase, "HONEY"))
      riscv_set_arch("RV32IMCXpulpv0");
    else if (strstr(uppercase, "GAP8"))
      riscv_set_arch("RV32IMCXgap8");
    else if (strstr(uppercase, "GAP9"))
      riscv_set_arch("RV32IMCXgap9");
    else if (strstr(uppercase, "HUA20"))
      riscv_set_arch("RV32IMCXgap9");
    else
      fprintf(stderr, _("Unrecognized mchip= : %s\n"), uppercase);
    if (comma)
      *comma = ',';
  } else {
    /* Invalid option.  */
...
static void parse_riscv_dis_options(const char *opts_in) {
...
static void arg_print(struct disassemble_info *info, unsigned long val,
...
static void print_insn_args(const char *d, insn_t l, bfd_vma pc,
  ...
  for (; *d != '\0'; d++) {
    ...
    case 'C': /* RVC */
      ...
      case 'c': /* RS1, constrained to equal sp */
        ...
        break;
      case 'V': /* RS2 */
        print(info->stream, "%s", riscv_gpr_names[EXTRACT_OPERAND(CRS2, l)]);
        break;
      case 'i':
      ...
      case 'T': /* floating-point RS2 */
        print(info->stream, "%s", riscv_fpr_names[EXTRACT_OPERAND(CRS2, l)]);
        break;
    ...
    case 'b':
      if (d[1] == '1') {
        info->target = (EXTRACT_ITYPE_IMM(l) << 1) + pc;
        ++d;
        (*info->print_address_func)(info->target, info);
      } else if (d[1] == '2') {
        info->target = (EXTRACT_I1TYPE_UIMM(l) << 1) + pc;
        ++d;
        (*info->print_address_func)(info->target, info);
      } else if (d[1] == '3') {
        print(info->stream, "%d", (int)EXTRACT_I1TYPE_UIMM(l));
        ++d;
      } else if (d[1] == '5') {
        print(info->stream, "%d", ((int)EXTRACT_I5TYPE_UIMM(l)) & 0x1F);
        ++d;
      } else if (d[1] == 'I') {
        print(info->stream, "%d", ((int)EXTRACT_I5_1_TYPE_IMM(l) << 27) >> 27);
        ++d;
      } else if (d[1] == 'i') {
        print(info->stream, "%d", ((int)EXTRACT_I5_1_TYPE_UIMM(l)) & 0x1F);
        ++d;
      } else if (d[1] == 's') {
        print(info->stream, "%d", ((int)EXTRACT_I6TYPE_IMM(l) << 26) >> 26);
        ++d;
      } else if (d[1] == 'u') {
        print(info->stream, "%d", ((int)EXTRACT_I6TYPE_IMM(l)) & 0x1F);
        ++d;
      } else if (d[1] == 'U') {
        print(info->stream, "%d", ((int)EXTRACT_I6TYPE_IMM(l)) & 0x0F);
        ++d;
      } else if (d[1] == 'f') {
        print(info->stream, "%d", ((int)EXTRACT_I6TYPE_IMM(l)) & 0x01);
        ++d;
      } else if (d[1] == 'F') {
        print(info->stream, "%d", ((int)EXTRACT_I6TYPE_IMM(l)) & 0x03);
        ++d;
      } else
        (*info->print_address_func)(info->target, info);
      break;
    ...
    case 'P':
      arg_print(info, EXTRACT_OPERAND(PRED, l), riscv_pred_succ,
                ARRAY_SIZE(riscv_pred_succ));
      break;

    case 'Q':
      arg_print(info, EXTRACT_OPERAND(SUCC, l), riscv_pred_succ,
                ARRAY_SIZE(riscv_pred_succ));
      break;
...
#undef DECLARE_CSR
    ...
    default:
      /* xgettext:c-format */
      print(info->stream, _("# internal error, undefined modifier (%c)"), *d);
      return;
    }
  }
}
...
static int riscv_disassemble_insn(bfd_vma memaddr, insn_t word,
                                  disassemble_info *info) {
...
void print_riscv_disassembler_options(FILE *stream) {
  fprintf(stream, _("\n\
